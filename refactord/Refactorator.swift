...
//
//  $Id: //depot/Refactorator/refactord/Refactorator.swift#9 $
//
...
@objc public class Refactorator: NSObject, RefactoratorRequest {
    ...
    public func refactorFile( filePath: String, byteOffset: Int32, old: String, logDir: String, plugin: RefactoratorResponse ) -> Int32 {
        NSLog( "refactord -- refactorFile: \(filePath) \(byteOffset) \(logDir)")
        xcode = plugin

        guard let argv = LogParser( logDir: logDir ).argumentsForFile( filePath ) else {
...
//        }
        ...
        for file in argv.filter( { $0.hasSuffix( ".swift" ) } ) {
            ...
            traceEntities( sourcekitd_response_get_value( resp ), old: old,
    ...
    func traceEntities( resp: sourcekitd_variant_t, old: String, file: String, contents: NSMutableData? ) {
...
//            }
            ...
            if entityUSR == self.usrToPatch {
                ...
                if let contents = contents, patch = entity.patchText( contents, value: old ) {
                    xcode.willPatchFile( file, line:entity.line, col: entity.col, text: patch )
                    self.patches.append( entity )
                }
           }

            self.traceEntities( dict, old: old, file: file, contents: contents )
            return true
        }
    }
